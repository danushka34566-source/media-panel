...
export default async function OGOverviewPage() {
  const [
    ...
    getMediaCached({ limit: 1 }).then(photos => photos[0])
      .catch(() => undefined),
    getMediaCached({ limit: 1, camera: cameraIcon }).then(photos => photos[0])
      .catch(() => undefined),
    getMediaCached({ limit: 1, tag })
      .catch(() => []),
    getMediaCached({ limit: 1, tag: TAG_FAVS })
      .catch(() => []),
    getMediaCached({ limit: 1, camera })
      .catch(() => []),
    getMediaCached({ limit: 1, film })
      .catch(() => []),
    getMediaCached({ limit: 1, focal })
      .catch(() => []),
  ]);

  return (
    <div className="grid gap-3 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {photoBasic && <MediaOGTile photo={photoBasic} />}
      {photoIcon && <MediaOGTile photo={photoIcon} />}
