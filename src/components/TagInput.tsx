...
export default function TagInput({
  ...
  const optionValuesNormalized = useMemo(() =>
    new Set(optionValues.map(option => parameterize(option)))
  , [optionValues]);

  const selectedOptions = useMemo(() =>
    convertStringToArray(value, shouldParameterize ?? false) ?? []
  , [value, shouldParameterize]);

  const selectedOptionsNormalized = useMemo(() =>
    new Set(selectedOptions.map(option => parameterize(option)))
  , [selectedOptions]);
  ...
  const normalizeValueForComparison = useCallback((option: string) =>
    shouldParameterize
      ? parameterize(option)
      : parameterize(option.trim()),
  [shouldParameterize]);

  const isInputTextUnique = useMemo(() => {
    const normalized = normalizeValueForComparison(inputTextFormatted);
    if (!inputTextFormatted || !normalized) { return false; }
    return !optionValuesNormalized.has(normalized) &&
      !selectedOptionsNormalized.has(normalized);
  }, [
    inputTextFormatted,
    normalizeValueForComparison,
    optionValuesNormalized,
    selectedOptionsNormalized,
  ]);
  ...
  const optionsFiltered = useMemo<AnnotatedTag[]>(() => hasReachedLimit
    ...
    : (isInputTextUnique && allowNewValues
      ...
      .filter(({ value, label }) =>{
        ...
        if (selectedOptionsNormalized.has(parameterize(value))) {
          return false;
        }
        if (!inputTextFormatted) { return true; }
        return shouldParameterize
          ? key.includes(inputTextFormatted)
          : (parameterize(key))
            .includes(parameterize(inputTextFormatted));
      }))
  , [
    hasReachedLimit,
    inputTextFormatted,
  ...
  const addOptions = useCallback((options: (string | undefined)[]) => {
    const seenNormalized = new Set(selectedOptionsNormalized);
    const optionsToAdd = (options
      ...
      .map(option => shouldParameterize
        ...
        : option)
      .filter(option => {
        const normalized = normalizeValueForComparison(option);
