...
const formatMediaForFeedRss = (photo: Media): FeedMediaRss => ({
  ...getCoreFeedFields(photo),
  link: absolutePathForMedia({ photo }),
  pubDate: photo.createdAt,
  media: {
    content: generateFeedMedia(photo, FEED_MEDIA_WIDTH_LARGE),
    thumb: generateFeedMedia(photo, FEED_MEDIA_WIDTH_MEDIUM),
  },
});

const feedMediaToXml = (photo: FeedMediaRss): string => {
  return `<item>
...
export const formatFeedRssXml = (photos: Media[]) =>
  `<?xml version="1.0" encoding="UTF-8"?>
   ...
   >
    <channel>
      ...
      ${photos.map(formatMediaForFeedRss).map(feedMediaToXml).join('\n')}
    </channel>
  </rss>`;
