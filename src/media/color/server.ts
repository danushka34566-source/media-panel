...
import { Oklch, MediaColorData } from './client';
...
const getImageDataFromUrl = async (_url: string) => {
  const url = getOptimizedMediaUrlForManipulation(_url, IS_PREVIEW);
...
export const getColorFieldsForMediaDbInsert = async (
...
export const getColorFieldsForMediaForm = async (
  ...args: Parameters<typeof getColorFieldsForImageUrl>
) => {
  const { colorSort, ...rest } =
    await getColorFieldsForMediaDbInsert(...args) ?? {};
