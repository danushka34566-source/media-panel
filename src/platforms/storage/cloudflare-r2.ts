...
export const cloudflareR2Copy = async (
  ...
  const destinationKey = addRandomSuffix
    ? `${name}-${generateStorageId()}.${extension}`
    : fileNameDestination;
  const encodedSourceKey = encodeURIComponent(fileNameSource)
    .replace(/%2F/g, '/');

  return cloudflareR2Client().send(new CopyObjectCommand({
    Bucket: CLOUDFLARE_R2_BUCKET,
    CopySource: `${CLOUDFLARE_R2_BUCKET}/${encodedSourceKey}`,
    Key: destinationKey,
  }))
    .then(() => urlForKey(destinationKey));
};
...
export const cloudflareR2ObjectExists = async (Key: string) => {
  try {
    await cloudflareR2Client().send(new HeadObjectCommand({
      Bucket: CLOUDFLARE_R2_BUCKET,
      Key,
    }));
    return true;
