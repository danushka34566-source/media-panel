...
export const descriptionForRecipeMedia = (
  photos: Media[] = [],
  appText: AppTextState,
  dateBased?: boolean,
  explicitCount?: number,
...
const photoHasRecipe = (photo?: Media) =>
  photo?.film && photo?.recipeData;

const getMediaWithRecipeFromMedia = (
  photos: Media[],
  preferredMedia?: Media,
) =>
  photoHasRecipe(preferredMedia)
    ? preferredMedia
    : photos.find(photoHasRecipe);

export const getRecipePropsFromMedia = (
  ...args: Parameters<typeof getMediaWithRecipeFromMedia>
): RecipeProps | undefined => {
  const photo = getMediaWithRecipeFromMedia(...args);
