...
export const descriptionForTaggedMedia = (
  photos: Media[] = [],
  appText: AppTextState,
  dateBased?: boolean,
  explicitCount?: number,
...
export const isMediaFav = ({ tags }: Media) => tags.some(isTagFavs);
...
export const convertTagsForForm = (
  ...
  sortTagsObjectWithoutFavs(tags)
    .map(({ tag, count }) => ({
      ...
      annotationAria:
        formatCountDescriptive(count, appText.category.taggedMedia),
    }));
