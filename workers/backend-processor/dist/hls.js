...
export const isHlsArtifactKey = (key) => /^[a-zA-Z0-9._@-]+-hls(?:-(?:high|720p))?(?:\.m3u8|-init\.mp4|-[0-9]{5}\.m4s)$/i.test(key);
export const parseHlsUris = (manifest) => {
    const uris = [];
    for (const m of manifest.matchAll(/#EXT-X-MAP:.*?URI="([^"]+)"/gi))
        if (m[1])
            uris.push(m[1]);
...
export const validateHlsVodManifest = (manifest, { allowAbsoluteUrls = false } = {}) => {
    if (!manifest.includes('#EXTM3U') || !/#EXT-X-PLAYLIST-TYPE:VOD/i.test(manifest) || !/#EXT-X-MAP:/i.test(manifest) || !/#EXT-X-ENDLIST/i.test(manifest))
        throw new Error('HLS manifest is not a complete fMP4 VOD playlist');
    const uris = parseHlsUris(manifest);
    if (uris.length < 2 || (!allowAbsoluteUrls && uris.some(uri => uri.includes('://') || uri.startsWith('/'))))
        throw new Error('HLS manifest contains invalid artifact URLs');
    return uris;
};
export const validateHlsMasterManifest = (manifest, { allowAbsoluteUrls = false } = {}) => {
    if (!manifest.includes('#EXTM3U') || !/#EXT-X-STREAM-INF:/i.test(manifest))
        throw new Error('HLS master manifest is incomplete');
    const uris = manifest.split(/\r?\n/).filter(line => line.trim() && !line.trim().startsWith('#'));
    if (uris.length < 1 || (!allowAbsoluteUrls && uris.some(uri => uri.includes('://') || uri.startsWith('/'))))
        throw new Error('HLS master contains invalid rendition URLs');
    return uris;
};
export const rewriteHlsManifestForStableUrls = (manifest, { initUrl, segmentUrlPrefix }) => manifest
    .replace(/(URI=")init\.mp4(")/g, `$1${initUrl}$2`)
    .replace(/(^|\r?\n)segment-(\d{5})(\.m4s)(?=\r?\n|$)/g, (_match, prefix, index, extension) => `${prefix}${segmentUrlPrefix}${index}${extension}`);
export const buildHlsMasterManifest = (renditions) => [
...
export const get720pDimensions = (sourceWidth, sourceHeight) => {
...
const generateRendition = async (inputPath, base, name, width, height, outputDirectory, durationSeconds, onProgress) => {
    ...
    for (const relativePath of ['init.mp4', ...uris.filter(uri => uri !== 'init.mp4')]) {
        if (!/^segment-\d{5}\.m4s$/i.test(relativePath) && relativePath !== 'init.mp4')
            throw new Error(`Unexpected HLS artifact: ${relativePath}`);
...
export const generateHlsVod = async (inputPath, base, outputDirectory, metadata, onProgress) => {
    const sourceWidth = Math.max(2, Math.floor(metadata.mediaWidth || 720));
    const sourceHeight = Math.max(2, Math.floor(metadata.mediaHeight || 404));
    const renditions = [await generateRendition(inputPath, base, 'high', sourceWidth - sourceWidth % 2, sourceHeight - sourceHeight % 2, path.join(outputDirectory, 'high'), metadata.durationSeconds, onProgress)];
    ...
    const masterPath = path.join(outputDirectory, 'index.m3u8');
    await fs.mkdir(outputDirectory, { recursive: true });
    await fs.writeFile(masterPath, '#EXTM3U\n');
    const master = { key: getHlsArtifactKeys(base).manifest, filePath: masterPath, contentType: 'application/vnd.apple.mpegurl', size: (await fs.stat(masterPath)).size };
