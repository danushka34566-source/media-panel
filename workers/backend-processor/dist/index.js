...
const normalizeBaseUrl = (value) => {
    ...
    if (!trimmed) {
        throw new Error('Missing BACKEND_ORCHESTRATOR_BASE_URL in backend processor env');
    }
    try {
        ...
        throw new Error(`Invalid BACKEND_ORCHESTRATOR_BASE_URL: "${trimmed}". ` +
            'Expected a full URL like https://your-worker.workers.dev');
    }
};
const BACKEND_ORCHESTRATOR_BASE_URL_NORMALIZED = normalizeBaseUrl(BACKEND_ORCHESTRATOR_BASE_URL);
const toArrayBuffer = (buffer) => buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength);
const orchestratorRequest = async (pathname, init) => {
    const response = await fetch(`${BACKEND_ORCHESTRATOR_BASE_URL_NORMALIZED}${pathname}`, {
        ...init,
        headers: {
...
const claimJobs = async () => orchestratorRequest(`/jobs/claim?limit=${CLAIM_LIMIT}`)
    .then(res => res.json())
    .then(data => data.jobs ?? []);
const runRegistrationJob = async () => orchestratorRequest('/registration/jobs/run', {
    method: 'POST',
...
const pullRegistrationJobs = async () => {
    ...
    const results = await Promise.allSettled(Array.from({ length: concurrency }, () => runRegistrationJob()));
    let registered = 0;
...
const refreshRuntimeConfigIfDue = async () => {
    const refreshIntervalMs = Math.max(POLL_INTERVAL_MS, IDLE_INTERVAL_MS, HEARTBEAT_INTERVAL_MS);
...
const getPreviewWindow = (durationSeconds) => {
    ...
    const midpoint = Math.max(windowStart + (windowEnd - windowStart) / 2, 0);
    const previewDuration = safeDuration > 0
        ? Math.min(PREVIEW_DURATION_SECONDS, Math.max(1, safeDuration * PREVIEW_MAX_DURATION_RATIO))
        : PREVIEW_DURATION_SECONDS;
    const previewSeek = Math.min(Math.max(midpoint - previewDuration / 2, 0), Math.max(safeDuration - previewDuration, 0));
    return {
        midpoint,
        previewSeek,
        previewDuration,
...
const probeVideo = async (inputPath) => new Promise((resolve, reject) => {
    ffmpeg(inputPath).ffprobe((error, data) => {
        ...
        const durationSeconds = parseMaybeNumber(videoStream?.duration) ??
            parseMaybeNumber(data?.format?.duration);
        ...
        resolve({
            ...
            subtitleTracks: getEmbeddedSubtitleTracks(data.streams),
        });
    });
});
...
const generateCanonicalMp4 = async (inputPath, outputPath, metadata, onProgress) => new Promise((resolve, reject) => {
...
const completeJob = async (job, metadata, posterBuffer, previewBuffer, subtitleFiles) => {
    ...
    subtitleFiles.forEach(track => {
        formData.append('subtitles', new File([toArrayBuffer(track.buffer)], track.fileName, {
            type: 'text/vtt',
        }));
    });
    formData.set('preview', new File([toArrayBuffer(previewBuffer)], `${job.fileNameBase}-preview.mp4`, {
        type: 'video/mp4',
    }));
...
let activeJobForShutdown;
let shutdownStarted = false;
const handleProcessorShutdown = async (signal) => {
    if (shutdownStarted) {
        return;
    }
    ...
    if (activeJobForShutdown) {
        await failJob(activeJobForShutdown, new Error(`Processor interrupted by ${signal}`));
    }
    process.exit(0);
};
process.once('SIGINT', () => { void handleProcessorShutdown('SIGINT'); });
process.once('SIGTERM', () => { void handleProcessorShutdown('SIGTERM'); });
const heartbeatJob = async (job, note) => orchestratorRequest('/jobs/heartbeat', {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
    },
...
const createFfmpegProgressReporter = ({ job, stage, durationSeconds, updateHeartbeat, }) => {
    let lastPercent = -1;
    let lastUpdateAt = 0;
    return progress => {
        ...
        const now = Date.now();
        if (percent === lastPercent || (percent < lastPercent + 2 && now - lastUpdateAt < 5_000)) {
            return;
        }
        lastPercent = percent;
        lastUpdateAt = now;
    ...
    try {
        ...
        for (let attempt = 1; attempt <= DOWNLOAD_ATTEMPTS; attempt += 1) {
            ...
            try {
                ...
                const progress = new Transform({
                    transform(chunk, _encoding, callback) {
                        ...
                        if (Number.isFinite(contentLength) && contentLength > 0) {
                            const percent = Math.min(99, Math.floor(downloadedBytes / contentLength * 100));
                ...
                await pipeline(Readable.fromWeb(response.body), progress, createWriteStream(inputPath));
                break;
            }
            catch (error) {
        ...
        if (job.canonicalOutputKey) {
            ...
            await generateCanonicalMp4(inputPath, canonicalPath, metadata, createFfmpegProgressReporter({
                job,
        ...
        if (compatibilityRequired &&
            ...
            await updateHeartbeat(streamStrategy === 'remux'
            ...
            await generateCompatibilityStream(inputPath, streamPath, metadata, createFfmpegProgressReporter({
                job,
            ...
            await uploadStreamDerivative({
                ...
                onProgress: (completed, total) => {
                    ...
                    if (percent !== lastProgressPercent) {
                        ...
                        void updateHeartbeat(`Uploading mobile-compatible stream: ${percent}%`);
                    }
                },
            });
        ...
        const subtitleFiles = await extractEmbeddedSubtitles(inputPath, job.fileNameBase, subtitleTracks);
