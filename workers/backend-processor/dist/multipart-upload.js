...
const requestJson = async (request, body) => request('/jobs/storage/multipart', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
}).then(response => response.json());
const uploadPart = async ({ filePath, start, end, url, }) => {
    for (let attempt = 0; attempt <= PART_RETRIES; attempt += 1) {
        ...
        try {
            ...
            if (!etag) {
                throw new Error('Stream part upload returned no ETag');
            }
            return etag;
        }
        catch (error) {
...
export const uploadStreamDerivative = async ({ request, filePath, key, photoId, contentType, onProgress, }) => {
    ...
    const plans = Array.from({ length: Math.max(1, Math.ceil(size / PART_SIZE_BYTES)) }, (_, index) => ({
        partNumber: index + 1,
    ...
    try {
        ...
        await Promise.all(Array.from({ length: Math.min(UPLOAD_CONCURRENCY, plans.length) }, () => worker()));
